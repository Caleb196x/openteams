...
你可能已经在用 Claude Code、Codex、Gemini CLI 或其他编程 Agent。它们都能规划、写代码、审查和测试。但只要一项工作涉及多个 Agent，**你就会变成它们之间的“消息总线”**：在终端之间搬运上下文、处理重叠的改动、确认到底完成了什么，再决定下一步交给谁。

openteams 补上的是这些 Agent 周围的协作层：**一个共享的工作空间、一套看得见的复杂任务执行流程，以及始终掌握在开发者手里的项目控制权。**
...
| 由开发者掌控的事项、隔离 worktree 和构建统计 | 由 Agent 决定的路线图，或只有消耗数字的 Token 计数器 |

**具体来说，安装后你会得到：** 用于轻量协作的直接聊天、用于计划执行的 Plan 模式、开箱即用的团队工作流程模板、把工作关联到会话且由开发者掌控的事项、用于隔离并行任务的可选 Git worktree，以及把交付结果与 Token 用量和成本放在一起展示的构建统计。

```text
你的编程 Agent               openteams

Claude Code ─┐              ┌─ 共享上下文
Codex ───────┼─────────────►├─ 可视化工作流 ── 审查 ── 合并
Gemini CLI ──┘              ├─ 隔离 worktree
                            └─ 事项 + 构建统计
```
