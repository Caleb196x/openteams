...
すでに Claude Code、Codex、Gemini CLI などのコーディング Agent を使っているでしょう。どれも単独なら問題なく働きます。そこで二つ目のターミナルを開き、さらに三つ目を開きます。同じコンテキストを繰り返し伝え、結果を別のウィンドウへ運び、誰が何を変更しているかを自分で覚えておきます。数日後には、コードは複数のセッションに散らばり、タスク一覧は別の場所にあり、使った Token で何が作られたのかを簡単に説明できなくなります。

openteams は、そうした Agent の周りに足りないものを補います。**Agent が会話して作業を引き継げる一つの共有スペース、開発者が確認して操作できる計画、そしてロードマップを Agent に渡さず、プロジェクトのタスクと Agent の成果を結ぶ軽量なローカル記録です。**

| openteams **であるもの** | openteams **ではないもの** |
| --- | --- |
| いま使っているコーディング Agent をつなぐローカルファーストのワークスペース | 新しいモデルや、Claude Code、Codex、Gemini CLI の代替品 |
| Agent が会話し、仕事を引き継ぎ、同じコンテキストを保てる共有セッション | 自分で調整し続ける必要がある、ばらばらのチャット画面 |
| 開発者が管理し、Agent のセッションに関連付ける Issue リスト | 本格的なプロジェクト管理ツールや、Agent が書き換えるロードマップ |
| ステップごとに確認、レビュー、中断、再試行できるワークフロー | 完了まで中身が見えない一つの大きなプロンプト |
| 個別にレビュー、マージ、破棄できる分離 worktree | 複数の Agent が同じワークスペースを変更して干渉し合う状態 |
| Agent の成果、使用量、コストが分かるビルド統計 | 何を構築したか分からない Token カウンター |

**インストールすると、具体的には次が使えます。** 軽い協業と計画的な実行に対応するチャットセッション、すぐに使えるチームワークフローテンプレート、作業をセッションに結び付けて開発者が管理する Issue、並行タスクを分離する独立ワークスペース、そして完全なビルド統計です。
...
Agent にコードを書かせること自体は、もう難しくありません。難しいのは、その仕事をきちんと管理することです。コンテキストが引き継がれているか、いま何が進んでいるか、並行作業がぶつからないか、次に何をするか、そしてどれだけコストがかかったかを把握する必要があります。

openteams は Agent と会話を一つのセッションにまとめます。複雑なタスクでは、Workflow モードがステップと依存関係を表示するため、すべてをやり直さずに必要な部分だけをレビュー、再試行できます。複数のセッションを同時に動かす場合は、それぞれに専用の Git worktree を用意し、未完成の変更を分けたまま、あとでマージまたは破棄できます。

プロジェクトの方向を決めるのは、あくまで開発者です。Issue には開発者が選んだ作業を記録し、Agent が実行するセッションを紐づけます。Agent は作業を進めますが、計画を書き換えることはありません。作業後は、ビルド統計で成果と Token 使用量、コストをまとめて確認できます。

openteams が目指すのは、Agent の数を増やすことではありません。何を作っているのか、変更はどこにあるのか、次に何をするのか、そしていくらかかったのかを、いつでも分かるようにすることです。
...
**openteams** には、組み込みの openteams CLI エージェントが含まれています。アプリ内の `setting->provider config->add provider` からモデルプロバイダーを設定できます。
