...
すでに Claude Code、Codex、Gemini CLI などのコーディング Agent を使っているかもしれません。どれも計画、実装、レビュー、テストができます。しかし、複数の Agent が一つの仕事に関わった途端、**あなた自身が Agent 間の中継役**になります。ターミナル間でコンテキストを運び、重複した変更を整理し、何が終わったかを確認して、次の担当を決めなければなりません。

openteams は、個々の Agent の周りに足りなかった協業の仕組みです。**作業場所を一つにまとめ、複雑なタスクの流れを見えるようにし、プロジェクトの判断は開発者の手元に残します。**
...
| 開発者が管理する Issue、分離 worktree、ビルド統計 | Agent が決めるロードマップや、消費量だけを示す Token カウンター |

**インストールすると、具体的には次が使えます。** 軽い協業向けのダイレクトチャット、計画的に実行する Plan モード、すぐに使えるチームワークフローテンプレート、作業とセッションを結びつけて開発者が管理する Issue、並行タスクを分離するオプションの Git worktree、そして成果を Token 使用量やコストと並べて確認できるビルド統計です。

```text
コーディング Agent            openteams

Claude Code ─┐               ┌─ 共有コンテキスト
Codex ───────┼──────────────►├─ 見えるワークフロー ── レビュー ── マージ
Gemini CLI ──┘               ├─ 分離 worktree
                             └─ Issue + ビルド統計
```
