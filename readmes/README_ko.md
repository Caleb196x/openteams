...
openteams는 이런 Agent 주변에 빠진 부분을 채웁니다. **Agent가 대화하고 작업을 넘길 수 있는 하나의 공유 공간, 개발자가 직접 보고 통제하는 계획, 그리고 로드맵을 Agent에게 넘기지 않으면서 프로젝트 작업과 Agent 결과물을 연결하는 가벼운 로컬 기록입니다.**
...
| 개발자가 관리하고 Agent 세션에 연결하는 이슈 목록 | 완전한 프로젝트 관리 제품이나 Agent가 다시 쓰는 로드맵 |
...
| Agent의 결과물, 사용량, 비용을 함께 보여 주는 빌드 통계 | 무엇을 만들었는지 알 수 없는 Token 카운터 |
...
**openteams**에는 내장 openteams CLI 에이전트가 포함되어 있습니다. 앱에서 `setting->provider config->add provider`로 모델 제공자를 설정하세요.
